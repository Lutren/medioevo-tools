...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--tui", action="store_true", help="Start split-screen TUI mode (Cerebro layout)")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not items:
        if args.tui:
            return _tui_interactive(config)
        options = ConversationEngineOptions(
            codex_provider=args.codex_provider,
            codex_timeout=args.codex_timeout,
            allow_cloud=_conversation_cloud_allowed(args),
...
def _tui_interactive(config: RuntimeConfig) -> int:
    """Start the split-screen TUI mode (Cerebro layout)."""
    try:
        from wabi_sabi.cli.tui import WabiTUI
    except ImportError as e:
        print(f"TUI mode requires 'rich' and 'prompt_toolkit': {e}")
    ...
    tui.add_message("system", "Wabi-Sabi TUI iniciado. Layout Cerebro activo.")
    tui.add_message("system", "Comandos: /model, /continue, /status, /providers, /help, /exit")
    tui.set_plan([
        "Cargar contexto de sesion (NEXT_SESSION_BRIEF.md, AGENTS.md)",
    ...
    def handle_input(cmd: str):
        ...
        if cmd == "/providers":
            tui.add_message("system", "Providers: nvidia, deepseek, cloudflare, ollama, openai, anthropic, gemini, qwen, groq, xai, mistral, cohere, together, fireworks, cerebras, huggingface, perplexity, deepinfra, sambanova")
            return

        if cmd == "/continue":
            tui.set_thinking("Reconstruyendo contexto desde handoff...")
            tui.add_session_ref("NEXT_SESSION_BRIEF.md")
            tui.add_session_ref("AGENTS.md")
            tui.add_session_ref("HANDOFF_CURRENT.md")
            tui.add_session_ref("05_HANDOFF_legacy/")
            time.sleep(0.5)
            tui.add_tool_log("read_file(NEXT_SESSION_BRIEF.md)")
            tui.add_tool_log("read_file(AGENTS.md)")
            tui.add_tool_log("read_file(HANDOFF_CURRENT.md)")
            tui.add_message("assistant", "Contexto reconstruido. Sesion anterior: 3 pendientes, 2 agentes (Opus, Sonet).")
            tui.clear_thinking()
            return

        if cmd == "/help":
            tui.add_message("system", "Comandos disponibles:\n  /model <alias> - Cambiar modelo\n  /continue - Reconstruir contexto desde handoff\n  /status - Mostrar estado actual\n  /providers - Listar providers\n  /plans - Toggle panel plan\n  /help - Esta ayuda\n  /exit - Salir")
            return
        ...
        # Default: execute as chat prompt
        tui.set_thinking("PROCESANDO...")
        try:
            payload = execute_chat_prompt(cmd, workspace=config.workspace, runtime_root=config.runtime_root)
            output = _format_auto_payload(payload)
            tui.add_message("assistant", output)
        except Exception as e:
            tui.add_message("system", f"[red]Error: {e}[/red]")
        finally:
            tui.clear_thinking()
