...
| `packages\open-dev\claudio-agent-runtime` | OPEN_LOCAL_P0_REVIEW | kernel local dry-run para DUAT Operator Shell; stdlib-only, tests `15 passed`, secret scan focalizado `0`, sin canales externos; WitnessLog, GhostGate plan y execute/rollback local limitado | riesgo de presentarlo como runtime autonomo listo o publicar antes de resolver licencia, claims, R/Phi budget y gates de canales externos | mantener local; no publicar, no instalar como servicio, no activar Telegram/GitHub/Spotify; avanzar solo con ActionGate y tests |
...
| `packages\paid\medioevo-agent-ops-pack` | COMMERCIAL_PUBLISHED | pack comercial de plantillas para operaciones con agentes, ActionGate, handoff, release checklist y listings; Gumroad `https://lrgonzalez.gumroad.com/l/medioevo-agent-ops-pack` | puede confundirse con garantia de seguridad autonoma, consultoria privada o runtime Claudio si el copy se exagera | vender solo como plantillas y ejemplos sinteticos; no incluir runtime privado, prompts, credenciales, libros completos ni RPG/TCG |
...
| `books\editorial` | BOOKS_EDITORIAL | muestras/borradores y exports internos controlados | riesgo de incluir libros completos; `internal_exports\DERIVA_INTERNAL_EXPORT_2026-05-17`, `FRAGMENTOS_INTERNAL_EXPORT_2026-05-22` y `CALIBRACION_INTERNAL_EXPORT_2026-05-22` son full-text interno | mantener all rights reserved; no copiar `internal_exports` a GitHub/Gumroad/web/KDP/ZIP publico sin gate separado |
...
| `PRODUCTOS_MEDIOEVO\01_LIBROS_Y_BUNDLES\despertar-preview-gumroad_20260513_232758` | COMMERCIAL_BOOKS_PUBLIC_SAFE_PREVIEW | muestra publica aprobada de DESPERTAR con boundary, listing Gumroad y product assets v1 | no afirmar bestseller real; no sustituye lectura legal/mercado; cualquier edicion futura de Gumroad/redes sigue gateada | mantener solo el preview publico, con direct link, assets aprobados y sin archivo completo |
...
## MEDIOEVO GM Lite private repo - 2026-05-14

| path | classification | reason | risk | recommended_action |
|---|---|---|---|---|
| `publish_staging\github-private\medioevo-gm-lite-private-mvp` / `https://github.com/Lutren/medioevo-gm-lite` | PRIVATE_REPO_LIVE | MVP web subido a repo GitHub privado con `private=true`, build y boundary check verificados | cambio accidental de visibilidad, deploy publico prematuro o inclusion futura de packs privados completos | mantener privado; no desplegar ni publicar sin gate separado y revision de contenido |

## MEDIOEVO OSIT Formal Lab - 2026-05-18

| path | classification | reason | risk | recommended_action |
|---|---|---|---|---|
| `publish_staging\open-dev\medioevo-osit-formal-lab` / `https://github.com/Lutren/medioevo-osit-formal-lab` | OPEN_PUBLIC_SAFE_CANDIDATE | laboratorio reproducible para `H_eff`, `R`, `Phi_eff`, residuo MTS y gates; tests y secret scan focalizado limpios | riesgo de sobreclaim si se presenta como fisica validada, conciencia, AGI, prediccion garantizada o sustituto de Shannon | publicar solo con `CLAIMS.md`, `PRIVATE_EXCLUSIONS.md`, licencia en `LEGAL_REVIEW_REQUIRED`, fixtures sinteticos y lenguaje de formal lab |
