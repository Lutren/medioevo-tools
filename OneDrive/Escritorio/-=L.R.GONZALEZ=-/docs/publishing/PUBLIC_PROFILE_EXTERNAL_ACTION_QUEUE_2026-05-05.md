...
|---|---|---|---|---|---|
| P0 | LinkedIn | visually confirm canonical profile URL and apply profile headline/about only if authenticated owner view is correct | `AUTHENTICATED_CONFIRMATION_REQUIRED` | fresh host `APPROVE`, exact owner-view target window, authenticated UI, no private data | public profile URL returns expected headline/about |
...
| P2 | Instagram/TikTok/YouTube | publish short visual posts from content calendar with rendered PNG assets | `DRAFT_READY_AFTER_GATE` | fresh host `APPROVE`, account auth, asset rights check | live URL/screenshot per post |
