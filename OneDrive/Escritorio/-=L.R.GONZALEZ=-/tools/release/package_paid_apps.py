...
def main() -> int:
    parser = argparse.ArgumentParser(description="Dry-run or create paid app release packages.")
    add_common_args(parser)
    parser.add_argument("--product", choices=[p.name for p in PRODUCTS if p.lane == "paid-apps"], action="append")
    parser.add_argument("--execute", action="store_true", help="actually write ZIP files")
    args = parser.parse_args()
    validate_root_arg(args)

    selected = [product_by_name(name) for name in args.product] if args.product else [p for p in PRODUCTS if p.lane == "paid-apps"]
    out_dir = ROOT / "releases" / "paid-apps"
