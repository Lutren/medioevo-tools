...
def main() -> int:
    parser = argparse.ArgumentParser(description="Dry-run or create paid template release packages.")
    add_common_args(parser)
    parser.add_argument("--product", choices=[p.name for p in PRODUCTS if p.lane == "paid-templates"], action="append")
    parser.add_argument("--execute", action="store_true")
    args = parser.parse_args()
    validate_root_arg(args)

    selected = [product_by_name(name) for name in args.product] if args.product else [p for p in PRODUCTS if p.lane == "paid-templates"]
    out_dir = ROOT / "releases" / "paid"
    for product in selected:
        ...
        with ZipFile(zip_path, "w", ZIP_DEFLATED) as zf:
            for path in files:
                zf.write(path, path.resolve().relative_to(product.source_path.resolve()).as_posix())
